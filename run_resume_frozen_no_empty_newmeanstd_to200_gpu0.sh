...
if [[ "${CUDA_VISIBLE_DEVICES}" != "0" ]]; then
  echo "ERROR: this script is fixed to GPU 0. Got CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
  exit 2
fi
...
OUT_DIR="${OUT_DIR:-${PROJECT_ROOT}/experiments/original_frozen_no_empty_newmeanstd_resume_to200_${timestamp}}"
mkdir -p "${OUT_DIR}"
CONSOLE_LOG="${OUT_DIR}/console_resume_to200_${timestamp}.log"
...
echo "Protocol: resume frozen train -> frozen test to 200 epochs; new output dir/log; old 100epoch logs/checkpoints are not overwritten"
