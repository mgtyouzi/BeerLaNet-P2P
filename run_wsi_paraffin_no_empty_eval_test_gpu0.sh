...
if [ -z "${CHECKPOINT:-}" ]; then
  LATEST_DIR="$(find "${PROJECT_ROOT}/experiments" -maxdepth 1 -type d -name 'wsi_paraffin_no_empty_100ep_*' | sort | tail -n 1 || true)"
  if [ -z "${LATEST_DIR}" ]; then
    echo "ERROR: no wsi_paraffin_no_empty_100ep_* experiment found."
...
echo "Protocol: final WSI-level paraffin test, empty-GT patches skipped."
