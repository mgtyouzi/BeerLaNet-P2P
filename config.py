...
_C.MODEL.LOSS = ['L2']
_C.MODEL.WEIGHT_DICT = {'loss_ce': 1.0, 'loss_points': 0.0, 'loss_aux': 0.0} 
...
_C.MODEL.AUX_kwargs = {'pos_coef': 1.0, 'neg_coef': 1.0, 'pos_loc': 0.0, 'neg_loc': 0.0}
...
# -----------------------------------------------------------------------------
_C.TEST = edict()
_C.TEST.MATCH_RADIUS = 15
_C.TEST.EVAL_MODE = 'cell'
    ...
    for k, v in a.items():
        ...
        if old_type is not type(v):
            if isinstance(b[k], np.ndarray):
                ...
                # 允许数字类型之间的转换，例如 int(200) 和 str('200')
                try:
                    # 尝试将新值转换为旧类型
                    if isinstance(b[k], (int, float)):
                        b[k] = old_type(v)
                        continue # 如果成功，跳过类型不匹配的错误
...
def cfg_from_file(filename):
    ...
    # 直接合并到全局 cfg
    global _C
    _C = cfg_merge_a2b(data, _C)


# ========================================================== #
# ======================【核心修改】========================= #
# 简化 cfg_from_list 和 merge_from_list 的逻辑
# ========================================================== #
def cfg_from_list(args_opts):
    ...
    assert len(args_opts) % 2 == 0, "Command line opts should be in pairs of KEY VALUE"
    
    for k, v in zip(args_opts[0::2], args_opts[1::2]):
        key_list = k.split('.')
        d = _C
        for subkey in key_list[:-1]:
            assert subkey in d, f"Config key '{subkey}' not found"
            d = d[subkey]
        
        subkey = key_list[-1]
        assert subkey in d, f"Config key '{subkey}' not found in '{'.'.join(key_list[:-1])}'"
        
        # 尝试将值转换为正确的类型
        try:
            # 尝试解析为 Python 字面量 (int, float, list, dict, bool, None)
...
def merge_from_list(cfg, args_opts):
    """
    这个函数现在直接调用 cfg_from_list，该函数会修改全局的 _C (也就是 cfg)。
    它不再需要返回任何东西或进行二次合并。
    """
    if args_opts:
        cfg_from_list(args_opts)
    return cfg
# ========================================================== #