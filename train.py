...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='/home/zhangwanqi/beijingertong_0213/pth/',
...
def test():
    ...
    # 加载预训练模型参数
    checkpoint_path = '/media/zhouzhihao/DeepInformatic_dataset/lihansheng/bxr/code/bxr_p2pnet/checkpoint/bxr_20230809_p2p.pth'
    checkpoint_dict = torch.load(checkpoint_path, map_location='cuda:0')

    # 打印checkpoint中的所有keys
    ...
    # 筛选出匹配的键并更新模型状态字典
    pretrained_dict = {k: v for k, v in checkpoint_dict['model'].items() if k in model_dict}
    ...
    # 打印数据集的一些基本信息以确认一致性
    print("Train dataset size:", len(data_loaders['train'].dataset))
    print("Test dataset size:", len(data_loaders['test'].dataset))

    # 打印评估设置
    print(f"Effective matching distance: {args.match_dis}")

    # 创建评估器并计算指标
    evaluator = Evaluator(data_loaders['test'], num_classes=4)
    metrics = evaluator.calculate_metrics(model, effective_matching_dis=args.match_dis, rank=0)
...
class Evaluator:
    ...
    def draw_roc(self, model, effective_matching_dis=12, rank=0):
        ...
        for i, (images, points, labels, lengths) in enumerate(self.data_loader):
            ...
            if len(pd_points) and len(gd_points):
                ...
                right_num, match_pred_index, match_gd_index = binary_match_predAndgd(pd_points, gd_points, threshold_distance=effective_matching_dis)
    ...
    def visual_analysis(self, model, output_dir='vis_results/normal'):
        ...
        for i, (images, points, labels, lengths) in enumerate(self.data_loader):
            ...
            template_db = r'/home/zhangwanqi/code/code_cell_p2p/slice.db'
            db_save_root = "/home/zhangwanqi/code/code_cell_p2p/employ/"
            table_name = "None"
            ...
            image = self.data_loader.dataset.read_data(self.data_loader.dataset.data[i], self.data_loader.dataset.files[i])['image'].copy()
            image1 = image.copy()
